...
class ExchangeAPI:
    ...
    def fetch_balance(self):
        try:
            ...
            balance_swap = round(float(balance_info[0]['total']), 4)
            balance_used = round(float(balance['used']['USDT']), 4)
            balance_unrealized = balance_info[0]['unrealised_pnl']
            cross_available = round(float(balance_info[0]['cross_available']), 4)
            return balance, balance_swap, cross_available, balance_info, balance_used,balance_unrealized
        except Exception as e:
